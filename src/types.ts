...
export type ActionCreatorsFromHandlers<T> = {
    [P in keyof T]: T[P] extends Handler<unknown, unknown, infer U> ? ActionCreator<U> : never
};

export type AsyncActionCreatorsFromEffects<T> = {
    [P in keyof T]: T[P] extends Effect<infer Params, infer Result> ? AsyncActionCreators<Params, Result, Error> : never
};
